...
for sub subdirectionary reperately 
"""
...
def word_count():
    ...
    folder_name = "dasovich-j"
    parent_path = "in/maildir/" + folder_name
    invalid_list = [] # folder has more than 5000 files
    stop_point = 0
    ...
    if stop_point == 0:
        ...
        with open('out/sub/spark-palindrome/145_swerzbin-m.json') as f:
            dic_palindrome = json.load(f)
        with open('out/sub/spark-anagram/145_swerzbin-m.json') as f:
    ...
    while (stop_point < len(names)):
        ...
        if name not in invalid_list:
            ...
            file_palindrome_path = "out/sub/spark-palindrome/" + str(folder_count) + "_" + name + ".json"
            with open(file_palindrome_path, 'w') as outfile:
                json.dump(dic_palindrome, outfile)
            file_anagram_path = "out/sub/spark-anagram/" + str(folder_count) + "_" + name + ".json"
            with open(file_anagram_path, 'w') as outfile:
                json.dump(dic_anagram, outfile)
    
    file_palindrome_path = "out/invalid/spark-palindrome/" + folder_name + ".json"
    with open(file_palindrome_path, 'w') as outfile:
        json.dump(dic_palindrome, outfile)
    file_anagram_path = "out/invalid/spark-anagram/" + folder_name + ".json"
    with open(file_anagram_path, 'w') as outfile:
        json.dump(dic_anagram, outfile)
