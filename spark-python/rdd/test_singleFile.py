...
def word_count():
    ...
    parent_path = "in/maildir/hernandez-j/notes_inbox"
    stop_point = 0
    ...
    names = [f for f in listdir(parent_path) if isfile(join(parent_path, f))]
    ...
    file_palindrome_path = "out/sub/sub/spark-palindrome/" + str(folder_count) + "_" + name + ".json"
    with open(file_palindrome_path, 'w') as outfile:
        json.dump(dic_palindrome, outfile)
    file_anagram_path = "out/sub/sub/spark-anagram/" + str(folder_count) + "_" + name + ".json"
